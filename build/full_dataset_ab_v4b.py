#!/usr/bin/env python3
"""V4b full A/B: V1 / V3 depheavy / V4 depheavy / V4b depheavy on all 4,340 images."""
...
print(f"new-in-V4 (not in V3): {sum(is_new)}", flush=True)
...
MODELS = {
    ...
    "V4b_depheavy": REPO / "export/FullDataSetProdV4b_saved_model/FullDataSetProdV4b_full_integer_quant_dwfix_depheavy_v3.tflite",
}
interps = {}
for name, mp in MODELS.items():
    ...
    print(f"loaded {name}: in_scale={in_det['quantization'][0]:.6f} in_zero={in_det['quantization'][1]}, out_scale={out_det['quantization'][0]:.6f} out_zero={out_det['quantization'][1]}", flush=True)
...
for i, p in enumerate(paths):
    ...
    if (i + 1) % 100 == 0:
        ...
        print(f"  {i+1}/{len(paths)}  {rate:.1f} img/s  ETA {eta/60:.1f} min", flush=True)

print(f"\ntotal time: {(time.monotonic()-t0)/60:.1f} min", flush=True)
...
report = {"n_total": len(paths), "per_model_overall": {}}
...
header_cols = "  ".join(f"{name:>28}" for name in MODELS)
print(f"{'cid':>5} {'size':>5}  {header_cols}")
report["per_cluster"] = {}
for cid in top_cids:
    ...
    print(f"{cid:>5} {size:>5}  " + "  ".join(f"{p:>28}" for p in parts))
...
print(f"{'REST':>5} {size_rest:>5}  " + "  ".join(f"{p:>28}" for p in parts))
...
(REPO / "build" / "full_dataset_ab_v4b.json").write_text(json.dumps(report, indent=2))
print(f"\n[done] wrote build/full_dataset_ab_v4b.json", flush=True)
